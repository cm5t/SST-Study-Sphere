...
# A large majority of this code was generated by Antigravity (to save time, not that I can't code this project myself), using many AI models (due to Quota Limit). The main ones were Gemini 3 Pro and 3 Flash, with abit of Claude Opus 4.6. On every implementation, I double checked that what the AI gave as its implementation plan is correct (Fulfils my requirements)
# All prompting and alpha-testing was done by me, README file (took inspiration from AI but paraphrased) (Dylan), and Supabase creation of columns, Streamlit secrets.toml are all mine. I did the pushing of this project to Github. All prompts was created by me. On some instances, I coded myself (Mainly for choosing of AI Model and instructions to give the AI) and guided the AI to solve some of the problems at times.
# I had the design idea ready before prompting the AI, including the features in the app (Forum posting, Leaderboard System with Levels and XP, and an AI Chatbot) and what UI it should have. When the AI did not perform to our expectations (either ruining the UI of the app, adding numerous errors to the code or the result not to our expectations), I always undid the changes that the AI made and tried to make the fixes on my own.
# Shao Kai created a draft app that I took some inspiration from.
# Username limit edge case and the bug where autologin logged all users into the same account was reported by Drake.
# yes i tried to credit my group members as much I can but thats it
# All comments was created by me (me suggesting the human, Dylan, not the AI).
...
# initialize Cookie Manager for autologin system
...
# Gemini AI Setup
# Configures the Google Generative AI SDK for AI Study Buddy and Autotagging. Uses the GOOGLE_API_KEY (mine) found in streamlit secrets.
def init_gemini():
...
def get_ai_tagging(description, file_bytes=None, mime_type=None):
    ...
    # Different models as one models quota may be exceeded.
    models_to_try = [
    ...
    for model_name in models_to_try:
        try:
            ...
            # Background info to ensure the AI doesn't go off track and context related to SST.
            prompt = """
            ...
            Level: [Level]
            """
            #adding the info for the ai to tag
            content = [prompt]
            if description:
    ...
    for line in response_text.split('\n'):
        ...
        # Suitability: Whether the uploaded content is appropriate for SST Study Sphere (not memes, jokes or whatever)
...
# Storage Bucket Setup: Ensures that a storage bucket called "file" exists and creates it if it doesn’t.
@st.cache_resource
...
# Cached Data Fetchers: These functions query Supabase and cache results using Streamlit's @st.cache_data to avoid redundant database calls on every rerun.
@st.cache_data(ttl=60)
def fetch_projects(query, subject, level, sort_by):
    """Fetch notes from the 'projects' table with optional filters and sorting."""
    # to decide which posts to show
...
# DataManager: A central class that encapsulates all database operations:
...
class DataManager:
    def __init__(self):
        # When you create an instance of said class, checking whether said user had votes stored in this session.
    ...
    def hash_password(self, password):
        """Hash a password with SHA-256 for storage and comparison. Ensures security such that both admins and hackers can't log in to your account"""
    ...
    def signup(self, email, username, password):
        ...
        try:
            ...
            # Above check is in place to not take up as much storage, and not make leaderboard/profile names too long such that it will stretch out the UI.
            existing_user = supabase.table("users").select("*").eq("username", username).execute()
            if existing_user.data:
                return False, "Username already taken."
            # Checks if the username is already taken, the email is already registered
            existing_email = supabase.table("users").select("*").eq("email", email).execute()
            if existing_email.data:
                return False, "Email already registered."

            hashed_pw = self.hash_password(password)
            # Creates a new user with the given email, username, and password. Initialises XP to 0.
            user_data = {
                "email": email,
                "username": username,
                "password": hashed_pw,
    ...
    def add_note(self, title, subject, level, description, uploaded_file):
        ...
        if uploaded_file:
            ...
            try:
                ...
                supabase.storage.from_(self.bucket_name).upload(
                    ...
                    # Upload said file to Supabase such that it can be downloaded by others
                )
        ...
        # Set params such that it can be uploaded to Supabase

        try:
            supabase.table("projects").insert(new_note).execute() # Upload project
            ...
            if author_data.data:
                ...
                supabase.table("users").update({"xp": new_xp}).eq("username", current_username).execute() # Increase XP of user
    ...
    def vote_note(self, note_id, current_score, note_author, vote_value):
        ...
        try:
            if existing is None:
                ...
                # Toggle: changing vote direction (Votes -= 2)
...
# CSS Styling: Defines the visual appearance for note cards, buttons, leaderboard etc
st.markdown("""
    <style>
...
# Session Persistence & Auto-Login: Saves/restores the logged-in user to auth_token.json so users stay logged in across browser reloads and app restarts.
def save_session_cookie(user_data):
    """Save user session to a signed cookie."""
    try:
        username = user_data['username']
        signed_username = sign_data(username)
        # Expires in 14 days such that when one gives away a computer, the login info/XP is not kept.
    ...
    try:
        ...
        if signed_token and isinstance(signed_token, str):
            ...
            if username:
                response = supabase.table("users").select("*").eq("username", username).execute()
                if response.data:
                    # If found, update info (Vote History, User, PW, XP, etc)
                    st.session_state.user = response.data[0]
...
if st.session_state.user is None:
        ...
        with st.form("login_form"):
            ...
            if submitted:
                if not email or not password:
                    # If no email and/or password is in, then prompt user to fill in them
    ...
    with tab_signup:
        # If one doesn't have an account
    ...
    with tab1:
        ...
        if st.session_state.focused_note_id is not None:
            # Focus Mode - Only one post is shown, options for Downloading, Liking are available
            focused_id = st.session_state.focused_note_id
            # Fetch fresh data for this note (Update votes)
            try:
            ...
            if note_data:
                note = note_data[0]
                score = note.get('votes', 0) # Fallback to 0 if no votes
                ...
                st.markdown("---")
                # Download a file if there is a file uploaded
            ...
            # Grid View: Browse & upload notes, sort which notes are seen, show the tags of the notes, able to click a button to toggle Focus Mode.
            ...
            with st.expander("⬆️ Upload New Note", expanded=True):
                ...
                # We use a key-based approach to clear the form manually. 'English' and 'Sec 1' are default values.
                def clear_form():
                    st.session_state.upload_key += 1 # Ensure that no 2 posts have the same ID, thus voting will increase the score of said post, not another post as well.
                ...
                with st.form(key=f"upload_form_{st.session_state.upload_key}", clear_on_submit=False):
                    ...
                    # Button: ask AI to detect Subject & Level from content (Autotagging System)
                    ...
                    # Show results of tags from autotagging system, and warn one if AI detected that post may not be suitable for the site to discourage people posting irrelevant stuff
                    if 'ai_result' in st.session_state:
                        ...
                        if not res["suitability"]:
                            ...
                            if st.form_submit_button("✅ Apply Suggested Tags"):
                                # It ensures that the tags are not forced, it is just a recommendation that the user can either accept it or tag it themselves.
            ...
            # Browse Posts (Forum part) -> 3 Columns
                    ...
                    for j in range(3):
                        with cols[j]:
                            if j < len(row_notes):
                                ...
                                # Color the vote score: green (+), red (-), gray (0) to highlight notes which are liked by the community -> and possibly relevant
                                if score > 0:
                                ...
                                if uploaded_name:
                                    ...
                                    file_tag = "📭 No File"
                                # Showing of post to user - Who made it? What subject/level is it? What does it contain?
                                st.markdown(f"""
                                    <div class="note-card">
                                        <h4>{note['title']}</h4>
    ...
    # TAB 2: Leaderboard: Ranks users by XP, showing their level and total XP.
    with tab2:
    ...
    # TAB 3: AI Study Buddy: Users pick a subject and type a topic. The AI fetches relevant notes from the forum and generates a study guide.
    with tab3:
